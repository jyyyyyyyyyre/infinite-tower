...
// --- 게임 데이터 테이블 ---
const itemData = {
    w001: { name: '낡은 단검', type: 'weapon', grade: 'Common',    baseEffect: 0.05, image: 'sword1.png' },
    a001: { name: '가죽 갑옷', type: 'armor',  grade: 'Common',    baseEffect: 0.05, image: 'armor1.png' },
    w002: { name: '강철 검', type: 'weapon', grade: 'Rare',      baseEffect: 0.12, image: 'sword2.png' },
    a002: { name: '판금 갑옷', type: 'armor',  grade: 'Rare',      baseEffect: 0.12, image: 'armor2.png' },
    w003: { name: '용살자 대검', type: 'weapon', grade: 'Legendary', baseEffect: 0.25, image: 'sword3.png' },
    a003: { name: '수호자의 갑주', type: 'armor',  grade: 'Legendary', baseEffect: 0.25, image: 'armor3.png' },
    w004: { name: '지배자의 롱소드', type: 'weapon', grade: 'Epic',      baseEffect: 0.50, image: 'sword4.png' },
    a004: { name: '영겁의 흉갑', type: 'armor',  grade: 'Epic',      baseEffect: 0.50, image: 'armor4.png' },
    w005: { name: '태초의 파편', type: 'weapon', grade: 'Mystic',    baseEffect: 2.50, image: 'sword5.png' },
    a005: { name: '세계수의 심장', type: 'armor',  grade: 'Mystic',    baseEffect: 2.50, image: 'armor5.png' },
};
const dropTable = {
    1: { itemsByGrade: { Common: ['w001', 'a001'] }, rates: { Common: 0.98, Rare: 0.02 } },
    2: { itemsByGrade: { Common: ['w001', 'a001'], Rare: ['w002', 'a002'], Legendary: ['w003', 'a003'] }, rates: { Common: 0.90, Rare: 0.09, Legendary: 0.01 } },
    3: { itemsByGrade: { Common: ['w001', 'a001'], Rare: ['w002', 'a002'], Legendary: ['w003', 'a003'], Epic: ['w004', 'a004'] }, rates: { Common: 0.78, Rare: 0.16, Legendary: 0.055, Epic: 0.005 } },
...
io.on('connection', async (socket) => {
    ...
    console.log(`[연결] 유저: ${socket.username} (ID: ${socket.userId})`);
...
function gameTick(player) {
    ...
    const mDmg = Math.max(0, m.attack - player.stats.total.defense);
    if(pDmg > 0 || mDmg > 0) {
        player.currentHp -= mDmg;
        player.monster.currentHp -= pDmg;
        player.socket.emit('combatResult', { playerTook: mDmg, monsterTook: pDmg });
    }
    if (player.currentHp <= 0) { resetPlayer(player, `[${player.level}층] 몬스터에게 패배하여 1층으로 귀환합니다.`); }
