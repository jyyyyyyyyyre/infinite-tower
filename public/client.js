...
function initializeGame(socket) {
    ...
    const getSellPrice = (grade) => SELL_PRICES[grade];
    ...
    const zoomLogic = {
        ...
        applyZoom(scale) {
            ...
            elements.zoom.gameContainer.style.setProperty('--game-scale', this.currentScale);
        },
        init() {
            elements.zoom.inBtn.addEventListener('click', () => {
                this.applyZoom(this.currentScale + 0.1);
            });
            elements.zoom.outBtn.addEventListener('click', () => {
                this.applyZoom(this.currentScale - 0.1);
            });
            this.applyZoom(1.0);
        }
    ...
    socket.on('rankingData', ({ topLevel, topGold, topWeapon, topArmor }) => {
        ...
        if (topLevel.length > 0) { rankingHTML += `<h3>🔝 최고 등반 랭킹</h3>`; topLevel.forEach((p, i) => { const content = `<span class="rank-name">${p.username}</span> 님은 최대 <span class="rank-value">${p.maxLevel}층</span>까지 등반하였습니다.`; rankingHTML += createRankItem(i + 1, content); }); }
        if (topWeapon.length > 0) { rankingHTML += `<h3>⚔️ 최고 무기 강화 랭킹</h3>`; topWeapon.forEach((p, i) => { const content = `<span class="rank-name">${p.username}</span> 님은 <span class="rank-value">${p.maxWeaponName}</span> 을(를) <span class="rank-value">${p.maxWeaponEnhancement}강</span>까지 강화하셨습니다.`; rankingHTML += createRankItem(i + 1, content); }); }
        if (topArmor.length > 0) { rankingHTML += `<h3>🛡️ 최고 방어구 강화 랭킹</h3>`; topArmor.forEach((p, i) => { const content = `<span class="rank-name">${p.username}</span> 님은 <span class="rank-value">${p.maxArmorName}</span> 을(를) <span class="rank-value">${p.maxArmorEnhancement}강</span>까지 강화하셨습니다.`; rankingHTML += createRankItem(i + 1, content); }); }
        if (topGold.length > 0) { rankingHTML += `<h3>💰 최고 골드 보유 랭킹</h3>`; topGold.forEach((p, i) => { const content = `<span class="rank-name">${p.username}</span> 님은 현재 <span class="rank-value">${p.gold.toLocaleString()} G</span> 를 보유하고 있습니다.`; rankingHTML += createRankItem(i + 1, content); }); }
    ...
    function renderGlobalRecords(records) {
        ...
        if (enhRecord) { enhDisplay.innerHTML = `<div class="record-item"><span class="rank-name">${enhRecord.username}</span> 님의 <span class="${enhRecord.itemGrade}">${enhRecord.itemName}</span> <span class="enhance-level-highlight">+${enhRecord.enhancementLevel}강</span></div>`; } else { enhDisplay.innerHTML = '<div class="no-record">아직 서버 최고 강화 기록이 없습니다.</div>'; }
        const lootDisplay = elements.modals.loot.display;
        let lootHTML = '';
        const gradeOrder = ['Mystic', 'Epic', 'Legendary'];
        let hasLootRecord = false;
        gradeOrder.forEach(grade => { const record = records[`topLoot_${grade}`]; if (record) { hasLootRecord = true; lootHTML += `<h3>🌟 ${grade} 등급 최고 기록</h3><div class="record-item"><span class="rank-name">${record.username}</span> 님이 <span class="${record.itemGrade}">${record.itemName}</span> 획득</div>`; } });
        if (hasLootRecord) { lootDisplay.innerHTML = lootHTML; } else { lootDisplay.innerHTML = '<div class="no-record">아직 서버 최고 득템 기록이 없습니다.</div>'; }
    }
    ...
    socket.on('onlineUsersData', (players) => {
        ...
        if (!players || players.length === 0) { list.innerHTML = '<li>현재 접속 중인 유저가 없습니다.</li>'; return; }
        const listHTML = players.map(p => { const userHTML = `<span class="rank-name">${p.username}</span>`; const weapon = p.weapon ? `<span class="user-item-name ${p.weapon.grade}">${p.weapon.name}</span>` : `<span class="user-item-none">맨손</span>`; const armor = p.armor ? `<span class="user-item-name ${p.armor.grade}">${p.armor.name}</span>` : `<span class="user-item-none">맨몸</span>`; const level = `<span class="rank-value">${p.level}층</span>`; return `<li>${userHTML} 님 : ${weapon}, ${armor} 을 착용하고, ${level} 등반 중</li>`; }).join('');
    ...
    const createItemHTML = (item) => {
        ...
        for (let i = 1; i <= item.enhancement; i++) { bonus += item.baseEffect * (i <= 10 ? 0.1 : 0.5); }
    ...
    const updateUI = ({ player, monster }) => {
        ...
        if (monster.isBoss) {
            elements.monster.level.innerHTML = `<span style="color:var(--fail-color); font-weight:bold;">${formatInt(monster.level)}층 보스</span>`;
        } else {
            elements.monster.level.textContent = `${formatInt(monster.level)}층 몬스터`;
        }
    ...
    const updateInteractionPanel = () => {
        ...
        const sellPrice = getSellPrice(item.grade);
        const isEquipped = currentPlayerState.equipment.weapon?.uid === item.uid || currentPlayerState.equipment.armor?.uid === item.uid;
    
        if (sellPrice && !isEquipped) {
            let sellHtml = `<div style="margin-top: 15px; border-top: 1px solid var(--border-color); padding-top: 15px;">
                                <span>판매 가격: ${sellPrice.toLocaleString()} G</span><br>`;
            sellHtml += `<button class="sell-btn" data-uid="${item.uid}" data-sell-all="false">1개 판매</button>`;
            if (item.enhancement === 0 && item.quantity > 1) {
                sellHtml += `<button class="sell-btn" data-uid="${item.uid}" data-sell-all="true">전체 (${item.quantity}개) 판매</button>`;
            }
            sellHtml += `</div>`;
            infoHTML += sellHtml;
        }
    ...
    const showDamagePopup = (target) => { const panel = elements[target]?.panel; if (!panel) return; panel.classList.add('hit-flash'); setTimeout(() => panel.classList.remove('hit-flash'), 100); };
    ...
    const playEnhancementAnimation = (result) => {
        ...
        switch (result) {
            ...
            default: text = '오류'; animClass = 'fail'; break;
        }
    ...
    socket.on('combatResult', (damages) => { if (damages.playerTook > 0) showDamagePopup('player'); if (damages.monsterTook > 0) showDamagePopup('monster'); });
    ...
    elements.tabs.buttons.forEach(btn => btn.addEventListener('click', () => { elements.tabs.buttons.forEach(b => b.classList.remove('active')); elements.tabs.contents.forEach(c => c.classList.remove('active')); btn.classList.add('active'); document.getElementById(btn.dataset.tab).classList.add('active'); }));
    ...
    elements.enhancement.button.addEventListener('click', () => { if (selectedInventoryItemUid) socket.emit('attemptEnhancement', selectedInventoryItemUid); });
    
    document.getElementById('enhancement-tab').addEventListener('click', (e) => {
        if (e.target.classList.contains('sell-btn')) {
            const uid = e.target.dataset.uid;
            const sellAll = e.target.dataset.sellAll === 'true';
            const item = currentPlayerState.inventory.find(i => i.uid === uid);
            if (!item) return;
            const quantity = sellAll ? item.quantity : 1;
            const price = getSellPrice(item.grade) * quantity;
            if (confirm(`[${item.name}] ${quantity}개를 ${price.toLocaleString()} G에 판매하시겠습니까?`)) {
    ...
    function addChatMessage(data) {
        ...
        if (isSystem) {
            ...
            item.classList.add(`${type}-message`);
